...
async function main(args) {
	...
	await preflightRange(numbers, args.force, circleFamilyConfig);

	for (const number of numbers) {
		const id = `${circleFamilyConfig.familyId}-${number}`;
		await createCircle({
			id,
	...
	console.log(
		`Created ${circleFamilyConfig.familyId} docs for range ${circleFamilyConfig.familyId}-${args.start} through ${circleFamilyConfig.familyId}-${args.end}`
	);
}
