...
import { createHex } from "./create-hex.mjs";
import { hexFamilyConfig } from "./scaffold/families/hex/presets.mjs";
...
async function main(args) {
	if (args.start === undefined || args.end === undefined) {
		throw new Error("Usage: pnpm create:hexes 2 5 [--force] [--examples glow,pattern-look]");
	}
	...
	await preflightRange(numbers, args.force, hexFamilyConfig);

	for (const number of numbers) {
		const id = `${hexFamilyConfig.familyId}-${number}`;
		await createHex({
			id,
			title: `${hexFamilyConfig.familyLabel} ${number}`,
			force: args.force,
	...
	console.log(
		`Created ${hexFamilyConfig.familyId} docs for range ${hexFamilyConfig.familyId}-${args.start} through ${hexFamilyConfig.familyId}-${args.end}`
	);
}
