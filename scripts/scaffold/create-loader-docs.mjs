...
function extractRuntimeDefaults(componentSource, fallbackValues) {
	...
	return {
		...
		speed: normalizeDocsDefault(extractDefaultValue(propsBlock, 'speed') ?? fallbackValues.speed),
...
import ${preset.importName}Raw from './use-cases/${preset.fileName}?raw';`
	...
	return `import type { InstallComponentDocs, PropsTable, ComponentDoc } from '$lib/types/structure';
...
const seo: SEO = {
	title: ${JSON.stringify(`${manifest.title} - SV Matrix Loader Component`)},
