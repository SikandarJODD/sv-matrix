...
import { createTriangle } from "./create-triangle.mjs";
import { triangleFamilyConfig } from "./scaffold/families/triangle/presets.mjs";
...
async function main(args) {
	if (args.start === undefined || args.end === undefined) {
		throw new Error(
			"Usage: pnpm create:triangles 2 5 [--force] [--examples glow,pattern-look]"
		);
	}
	...
	await preflightRange(numbers, args.force, triangleFamilyConfig);

	for (const number of numbers) {
		const id = `${triangleFamilyConfig.familyId}-${number}`;
		await createTriangle({
			id,
			title: `${triangleFamilyConfig.familyLabel} ${number}`,
			force: args.force,
	...
	console.log(
		`Created ${triangleFamilyConfig.familyId} docs for range ${triangleFamilyConfig.familyId}-${args.start} through ${triangleFamilyConfig.familyId}-${args.end}`
	);
}
