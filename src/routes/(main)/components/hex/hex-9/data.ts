...
import Hex9Raw from "$lib/components/loaders/hex/hex-9.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-9");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-9.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex9Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-9.svelte.",
		props: [
			{
				name: "size",
				type: "number",
