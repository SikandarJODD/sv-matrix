...
import Hex4Raw from "$lib/components/loaders/hex/hex-4.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-4");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-4.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex4Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-4.svelte.",
		props: [
			{
				name: "size",
				type: "number",
