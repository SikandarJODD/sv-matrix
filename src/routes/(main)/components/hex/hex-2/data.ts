...
import Hex2Raw from "$lib/components/loaders/hex/hex-2.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-2");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-2.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex2Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-2.svelte.",
		props: [
			{
				name: "size",
				type: "number",
