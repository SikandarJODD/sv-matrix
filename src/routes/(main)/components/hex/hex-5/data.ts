...
import Hex5Raw from "$lib/components/loaders/hex/hex-5.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-5");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-5.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex5Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-5.svelte.",
		props: [
			{
				name: "size",
				type: "number",
