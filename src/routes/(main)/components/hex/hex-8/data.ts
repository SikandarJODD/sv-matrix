...
import Hex8Raw from "$lib/components/loaders/hex/hex-8.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-8");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-8.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex8Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-8.svelte.",
		props: [
			{
				name: "size",
				type: "number",
