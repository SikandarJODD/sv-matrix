...
import Hex7Raw from "$lib/components/loaders/hex/hex-7.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-7");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-7.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex7Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-7.svelte.",
		props: [
			{
				name: "size",
				type: "number",
