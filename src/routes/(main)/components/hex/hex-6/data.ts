...
import Hex6Raw from "$lib/components/loaders/hex/hex-6.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-6");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-6.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex6Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-6.svelte.",
		props: [
			{
				name: "size",
				type: "number",
