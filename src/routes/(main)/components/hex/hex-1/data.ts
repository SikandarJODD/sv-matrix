...
import Hex1Raw from "$lib/components/loaders/hex/hex-1.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-1");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-1.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex1Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-1.svelte.",
		props: [
			{
				name: "size",
				type: "number",
