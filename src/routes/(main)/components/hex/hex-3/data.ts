...
import Hex3Raw from "$lib/components/loaders/hex/hex-3.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-3");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-3.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex3Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-3.svelte.",
		props: [
			{
				name: "size",
				type: "number",
