...
import Hex10Raw from "$lib/components/loaders/hex/hex-10.svelte?raw";
import { hexItems } from "$lib/content/matrix-navigation";

const navItem = hexItems.find((item) => item.id === "hex-10");

if (!navItem) {
	throw new Error("Missing navigation metadata for hex-10.");
}
...
const props: PropsTable[] = [
	{
		name: "Hex10Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by hex-10.svelte.",
		props: [
			{
				name: "size",
				type: "number",
				default: "34",
				description:
					"Overall hex matrix span in pixels before any optional wrapper scaling.",
			},
			{
				name: "dotSize",
				type: "number",
...
const seo: SEO = {
	...
	keywords: ["svelte loader", "hex loader", "dot matrix", "svelte component"],
};
