...
import Triangle5Raw from "$lib/components/loaders/triangle/triangle-5.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-5");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-5.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle5Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-5.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-5.svelte",
			filecode: Triangle5Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
