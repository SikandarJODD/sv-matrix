...
import Triangle11Raw from "$lib/components/loaders/triangle/triangle-11.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-11");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-11.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle11Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-11.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-11.svelte",
			filecode: Triangle11Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
