...
import Triangle19Raw from "$lib/components/loaders/triangle/triangle-19.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-19");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-19.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle19Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-19.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-19.svelte",
			filecode: Triangle19Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
