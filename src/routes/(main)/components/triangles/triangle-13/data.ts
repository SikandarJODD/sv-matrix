...
import Triangle13Raw from "$lib/components/loaders/triangle/triangle-13.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-13");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-13.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle13Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-13.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-13.svelte",
			filecode: Triangle13Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
