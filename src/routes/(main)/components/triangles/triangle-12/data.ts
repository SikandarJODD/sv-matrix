...
import Triangle12Raw from "$lib/components/loaders/triangle/triangle-12.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-12");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-12.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle12Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-12.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-12.svelte",
			filecode: Triangle12Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
