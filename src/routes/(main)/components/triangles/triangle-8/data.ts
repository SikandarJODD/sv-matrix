...
import Triangle8Raw from "$lib/components/loaders/triangle/triangle-8.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-8");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-8.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle8Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-8.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-8.svelte",
			filecode: Triangle8Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
