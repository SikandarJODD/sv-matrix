...
import Triangle20Raw from "$lib/components/loaders/triangle/triangle-20.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-20");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-20.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle20Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-20.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-20.svelte",
			filecode: Triangle20Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
