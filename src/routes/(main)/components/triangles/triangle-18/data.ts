...
import Triangle18Raw from "$lib/components/loaders/triangle/triangle-18.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-18");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-18.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle18Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-18.svelte.",
		props: [
			{
				name: "size",
				type: "number",
				default: "30",
				description:
					"Overall triangle matrix span in pixels before any optional wrapper scaling.",
			},
			{
				name: "dotSize",
				type: "number",
			...
			},
			{
				...
				default: "true",
				description:
					"Enables the default loading animation when reduced motion is not active.",
			},
			{
				name: "hoverAnimated",
				type: "boolean",
				default: "false",
				description:
					"Switches animation control to hover interactions instead of always running.",
			},
			{
				name: "color",
				type: "string",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-18.svelte",
			filecode: Triangle18Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
...
const seo: SEO = {
	...
	keywords: ["svelte loader", "triangle loader", "dot matrix", "svelte component"],
};
