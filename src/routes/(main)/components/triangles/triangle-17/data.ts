...
import Triangle17Raw from "$lib/components/loaders/triangle/triangle-17.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-17");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-17.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle17Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-17.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-17.svelte",
			filecode: Triangle17Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
