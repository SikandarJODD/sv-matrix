...
import Triangle4Raw from "$lib/components/loaders/triangle/triangle-4.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-4");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-4.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle4Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-4.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-4.svelte",
			filecode: Triangle4Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
