...
import Triangle9Raw from "$lib/components/loaders/triangle/triangle-9.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-9");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-9.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle9Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-9.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-9.svelte",
			filecode: Triangle9Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
