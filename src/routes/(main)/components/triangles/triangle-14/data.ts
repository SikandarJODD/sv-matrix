...
import Triangle14Raw from "$lib/components/loaders/triangle/triangle-14.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-14");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-14.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle14Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-14.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-14.svelte",
			filecode: Triangle14Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
