...
import Triangle1Raw from "$lib/components/loaders/triangle/triangle-1.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-1");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-1.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle1Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-1.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-1.svelte",
			filecode: Triangle1Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
