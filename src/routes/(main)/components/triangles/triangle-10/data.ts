...
import Triangle10Raw from "$lib/components/loaders/triangle/triangle-10.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-10");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-10.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle10Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-10.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-10.svelte",
			filecode: Triangle10Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
