...
import Triangle16Raw from "$lib/components/loaders/triangle/triangle-16.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-16");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-16.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle16Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-16.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-16.svelte",
			filecode: Triangle16Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
