...
import Triangle3Raw from "$lib/components/loaders/triangle/triangle-3.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-3");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-3.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle3Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-3.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-3.svelte",
			filecode: Triangle3Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
