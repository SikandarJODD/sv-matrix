...
import Triangle6Raw from "$lib/components/loaders/triangle/triangle-6.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-6");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-6.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle6Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-6.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-6.svelte",
			filecode: Triangle6Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
