...
import Triangle15Raw from "$lib/components/loaders/triangle/triangle-15.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-15");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-15.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle15Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-15.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-15.svelte",
			filecode: Triangle15Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
