...
import Triangle7Raw from "$lib/components/loaders/triangle/triangle-7.svelte?raw";
import { triangleItems } from "$lib/content/matrix-navigation";

const navItem = triangleItems.find((item) => item.id === "triangle-7");

if (!navItem) {
	throw new Error("Missing navigation metadata for triangle-7.");
}
...
const props: PropsTable[] = [
	{
		name: "Triangle7Props",
		desc: "Primary loader props. Defaults shown here are the effective defaults applied by triangle-7.svelte.",
		props: [
			{
				name: "size",
				type: "number",
...
const installBlock: InstallComponentDocs = {
	installCode: [
		{
			filename: "triangle-7.svelte",
			filecode: Triangle7Raw,
			lang: "svelte",
			isExpand: true,
		},
	],
